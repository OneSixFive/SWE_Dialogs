...
class Database:
    ...
    def backfill_lesson_progress_from_completed_sessions(self, *, apply: bool = False) -> dict[str, int]:
        count_query = """
            SELECT COUNT(*) AS count
            FROM lesson_sessions
            LEFT JOIN lesson_progress
                ON lesson_progress.user_id = lesson_sessions.user_id
                AND lesson_progress.lesson_id = lesson_sessions.lesson_id
            WHERE lesson_sessions.deleted_at IS NULL
                AND (lesson_sessions.is_completed = 1 OR lesson_sessions.status = 'completed')
                AND (
                    lesson_progress.id IS NULL
                    OR lesson_progress.is_completed = 0
        ...
        backfill_query = """
            ...
            SELECT
                lesson_sessions.user_id,
                lesson_sessions.lesson_id,
                'completed',
                1,
                COALESCE(
                    lesson_sessions.completed_at,
                    lesson_sessions.client_updated_at,
                    lesson_sessions.server_updated_at
                ),
                lesson_sessions.client_updated_at,
                lesson_sessions.server_updated_at
            FROM lesson_sessions
            ...
            DO UPDATE SET
                ...
                client_updated_at = excluded.client_updated_at,
                server_updated_at = excluded.server_updated_at
            WHERE lesson_progress.is_completed = 0 OR lesson_progress.status <> 'completed'
        """

        with self._connect() as connection:
            candidate_row = connection.execute(count_query).fetchone()
            candidate_count = int(candidate_row["count"])
            changed_count = 0
            if apply and candidate_count:
                before_changes = connection.total_changes
                connection.execute(backfill_query)
                changed_count = connection.total_changes - before_changes
                connection.commit()
            remaining_row = connection.execute(count_query).fetchone()
            remaining_count = int(remaining_row["count"])
